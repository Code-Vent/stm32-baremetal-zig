...
pub fn remap_i2c1() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 1); // Set I2C1_REMAP bit
}

pub fn remap_usart1() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 2); // Set USART1_REMAP bit
}

pub fn remap_usart2() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 3); // Set USART2_REMAP bit
}

pub fn remap_usart3_partial() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 4); // Set USART3_REMAP[1:0] to 01 (partial remap)
    mapr.* &= ~@as(u32, 1 << 5);
}

pub fn remap_usart3_full() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 4); // Set USART3_REMAP[1:0] to 11 (full remap)
    mapr.* |= @as(u32, 1 << 5);
}

fn remap_tim2_partial() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 8); // Set TIM2_REMAP[1:0] to 01 (partial remap)
    mapr.* &= ~@as(u32, 1 << 9);
}

fn remap_tim2_full() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 8); // Set TIM2_REMAP[1:0] to 10 (full remap)
    mapr.* |= @as(u32, 1 << 9);
}

fn remap_tim3_partial() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 11); // Set TIM3_REMAP[1:0] to 01 (partial remap)
    mapr.* &= ~@as(u32, 1 << 10);
}

fn remap_tim3_full() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 10); // Set TIM3_REMAP[1:0] to 11 (full remap)
    mapr.* |= @as(u32, 1 << 11);
}
...
fn remap_tim4_full() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 12); // Set TIM4_REMAP bit
}

pub fn remap_can() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 13); // Set CAN_REMAP bit
}

pub fn remap_pd01() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 15); // Set PD01_REMAP bit
}

pub fn remap_adc1_exttrig() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 17); // Set ADC1_ETRGINJ_REMAP bit
}

pub fn remap_adc2_exttrig() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 18); // Set ADC2_ETRGINJ_REMAP bit
}

pub fn remap_sw_jtag() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 24); // Set SWJ_CFG[2:0] to 010 (SW-DP enabled, JTAG-DP disabled)
    mapr.* &= ~(@as(u32, 1 << 25) | @as(u32, 1 << 26));
}

pub fn remap_sw_jtag_no_njtrst() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 24); // Set SWJ_CFG[2:0] to 100 (SW-DP enabled, JTAG-DP disabled, NJTRST disabled)
    mapr.* |= @as(u32, 1 << 26);
    mapr.* &= ~@as(u32, 1 << 25);
}
...
pub fn remap_full_jtag() void {
    const mapr = afio_reg(0x04);
    mapr.* &= ~@as(u32, (1 << 24) | (1 << 25) | (1 << 26)); // Set SWJ_CFG[2:0] to 000 (Full SWJ (JTAG-DP + SW-DP) enabled)
}

pub fn remap_no_jtag() void {
    const mapr = afio_reg(0x04);
    mapr.* |= (1 << 24); // Set SWJ_CFG[2:0] to 001 (JTAG-DP disabled and SW-DP enabled)
    mapr.* &= ~@as(u32, (1 << 25) | (1 << 26));
}

pub fn remap_eventout() void {
    const mapr = afio_reg(0x04);
    mapr.* |= @as(u32, 1 << 22); // Set EVENTOUT_REMAP bit
}

pub fn remap_can2() void {
    const mapr2 = afio_reg(0x1C);
    mapr2.* |= @as(u32, 1 << 9); // Set CAN2_REMAP bit
}

pub fn remap_fsmc_nadv() void {
    const mapr2 = afio_reg(0x1C);
    mapr2.* |= @as(u32, 1 << 23); // Set FSMC_NADV_REMAP bit
}

pub fn remap_eth() void {
    const mapr2 = afio_reg(0x1C);
    mapr2.* |= @as(u32, 1 << 28); // Set ETH_REMAP bit
}

pub fn remap_can3() void {
    const mapr2 = afio_reg(0x1C);
    mapr2.* |= @as(u32, 1 << 30); // Set CAN3_REMAP bit
}
